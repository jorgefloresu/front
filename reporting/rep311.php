...
function GetSalesInvoices($from, $to, $category=0, $location=null, $backorder=0, $debtor)
{
	...
	$sql .= " ORDER BY ".TB_PREF."debtor_trans.reference";
...
//	$cols = array(0, 60, 150, 260, 325,	385, 450);
	...
	$headers = array('', _('Code'),	_('Proveedor'), _('Venta COP'),	_('Compra USD'),
		'', '');
	...
	$rep = new FrontReport(_('Reporte de Ventas con su Valor de Compra'), "SalesVrsPurchases", user_pagesize(), 9, $orientation);
    if ($orientation == 'L')
...
// 		
		...
		$rep->AmountCol(4, 5, $myrow['valor_proveedor'], $dec);
