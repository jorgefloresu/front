...
*/
        $sql = "SELECT DISTINCT ".TB_PREF."suppliers.supp_name AS supplier_name, 
                ...
                FROM (
                        ...
                        AND (".TB_PREF."stock_master.mb_flag='B' OR ".TB_PREF."stock_master.mb_flag='M')) AS result, ".TB_PREF."suppliers, ".TB_PREF."stock_moves                       
                WHERE result.supplier_idx = ".TB_PREF."suppliers.supplier_id 
                AND result.stock_id=".TB_PREF."stock_moves.stock_id
                AND (".TB_PREF."stock_moves.type=".ST_SUPPRECEIVE." OR ".TB_PREF."stock_moves.type=".ST_SUPPCREDIT.")";
                
		if ($category != 0)
			$sql .= " AND result.category_id = ".db_escape($category);
		if ($location != '')
			$sql .= " AND result.loc_code = ".db_escape($location);
		if ($fromsupp != '')
			$sql .= " AND result.supplier_idx = ".db_escape($fromsupp);
		if ($item != '')
			$sql .= " AND result.stock_id = ".db_escape($item);

                $sql .= " ORDER BY result.category_id, supplier_name, result.tran_date, result.stock_id";
...
function print_inventory_purchase()
{
	...
	$headers = array(_('Category'), _('Description'), _('Date'), _('#'), _('Supplier'), _(' '), _('Price(USD) '), _(' Venta(COP)'));
	if ($fromsupp != '')
		$headers[4] = '';
    ...
    $rep = new FrontReport(_('Reporte de Ventas con Precios de Compra'), "InventoryPurchasingReport", user_pagesize(), 9, $orientation);
    if ($orientation == 'L')
...
// 		
		...
		$rep->AmountCol(6, 7, $trans['comision'], $dec);
		$amt = $trans['price_orig'] ; //$amt = $trans['qty'] * $trans['price'];
		$rep->AmountCol(7, 8, $amt, $dec);
		$rep->fontSize += 2;
		$total += $amt;
                $total_unit2 += $trans['comision'];
                $total_unit3 += $trans['comision'];
		$total_supp += $amt;
                $total_unit += $trans['comision'];
		$grandtotal += $amt;
		$total_qty += $trans['qty'];
                $rep->row -= 40;
        }
