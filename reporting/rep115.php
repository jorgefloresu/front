...
function check_outstanding($debtorno, $from, $to)
{
	$from = date2sql($from);
	$to = date2sql($to);
    ...
    $sql = "SELECT SUM(".TB_PREF."debtor_trans.ov_amount + ".TB_PREF."debtor_trans.ov_gst + ".TB_PREF."debtor_trans.ov_freight + 
    	...
    	GROUP BY ".TB_PREF."debtor_trans.debtor_no";

    $result = db_query($sql,"No result was returned");
...
function print_customer_balances()
{
	...
	{
		$pending_amount = check_outstanding($myrow['debtor_no'], $from, $to);
		if ($no_zeros && $pending_amount['SumOutstanding'] == 0) continue;
		...
		$bal = get_open_balance($myrow['debtor_no'], $from);
                $contact = get_branch_contacts('',null,$myrow['debtor_no']);
		$init[0] = $init[1] = 0.0;
		...
		//$rep->TextCol(3, 4,	_("Open Balance"));
                $rep->TextCol(3, 4,	$contact['phone']);
		//$rep->AmountCol(3, 4, $bal['charges'], $dec);
