...
function getTaxTransactions($from, $to)
{
	...
	$sql = "SELECT tt.name as taxname, taxrec.*, taxrec.amount*ex_rate AS amount,
		...
		WHERE (taxrec.amount <> 0 OR taxrec.net_amount <> 0)
			...
			ORDER BY taxrec.trans_type, name, ABS(taxrec.rate) DESC, taxname";
		//	ORDER BY taxrec.trans_type, name, br_name, taxrec.ex_rate";
...
function print_tax_report()
{
	...
	$cols = array(0, 160, 170, 220, 270, 300, 350, 400, 410);
	//$cols = array(0, 80, 130, 180, 270, 350, 400, 430, 480, 485, 540);

	//$headers = array(_('Trans Type'), _('Ref'), _('Date'), _('Name'), _('Branch Name'),
	//	_('Net'), _('Rate'), _('Tax'), '', _('Name'));
	$headers = array(_('Name'), '',
			_('Net'), _('Rate'), _('Tax'), '', _('Name'));
	...
	while ($trans=db_fetch($transactions))
	{
		...
		if (!$summaryOnly)
		{
			...
			//if ($trans['memo'] != $ant_memo) {
			//	$ant_memo = $trans['memo'];
			//if ($trans['br_name'] != $ant_brname) {
			//	$ant_brname = $trans['br_name'];
			//	$rep->TextCol(0, 1,	$trans['br_name']);
				//$rep->NewLine();
			//}
			if ($trans['name'] != $ant_name){
				if ($ant_taxname != '') {
					$rep->AmountCol(3, 4, $ant_rate, $dec);
					$rep->TextCol(5, 7, $ant_taxname);
					$rep->AmountCol(4, 9, $subtotal_impuesto, $dec);
				}
				$ant_name = $trans['name'];
				$rep->NewLine();
				...
				$subtotal_impuesto = 0.0;
				$ant_rate = $trans['rate'];
				$ant_taxname = $trans['taxname'];
			}
			if ($trans['taxname'] != $ant_taxname) {
				$rep->AmountCol(3, 4, $ant_rate, $dec);
				$rep->TextCol(5, 7, $ant_taxname);
				$rep->AmountCol(4, 9, $subtotal_impuesto, $dec);
				$subtotal_impuesto = 0.0;
			...
			$subtotal_impuesto += $trans['amount'];
			$rep->NewLine();
